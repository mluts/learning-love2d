...
(fn xt [x-coord] (/ x-coord (love.graphics.getWidth)))
(fn yt [y-coord] (/ y-coord (love.graphics.getHeight)))
...
(fn new-v [push acc stop-acc dt v min-v max-v]
  (if
    ...
    (< push 0) (clamp (when max-v (- max-v)) (when min-v (- min-v)) (- v (* acc dt)))
    (= 0 v) v
    (< v 0) (clamp (when max-v (- max-v)) (when min-v (- min-v)) (+ v (* stop-acc dt)))
